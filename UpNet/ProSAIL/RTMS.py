...
@author: raul modified by Dash
Using numba, and simplify the soil model to simple mixture model.
"""
...
def getPROSPECTcoef():
    import shelve
    pr=shelve.open('D:\\code\\retrieval_method\\ProSAIL\\PROSPECT_coef_v5',flag='r')
    data=pr['data']
    pr.close()
    return data
...
def CampbellLIDF(ALA):
    ...
    tx2=np.array([0,10,20,30,40,50,60,70,80,82,84,86,88], dtype = np.float32)
    tx1=np.array([10,20,30,40,50,60,70,80,82,84,86,88,90], dtype = np.float32)
    tl1=tx1*np.pi/180
    tl2=tx2*np.pi/180
...
def fDir(wvl):
    fdir=skylspec(wvl,0.19).reshape((-1,1))
    return fdir

# Initialization
prospect_coeff = getPROSPECTcoef()
soil_coef = getSoilSpeccoef(np.arange(400, 2501, 1.0))
fdir_all_wvl = fDir(np.arange(400, 2501, 1.0))
# soil_spectraf = pkgutil.get_data("prosail", "D:\code\retrieval_method\ProSAIL\soil_reflectance.txt")
soil_refs = np.loadtxt("D:\\code\\retrieval_method\\ProSAIL\\soil_reflectance.txt")
rsoil1 = soil_refs[:, 0]
rsoil2 = soil_refs[:, 1]

@jit(nopython = True)
def generate_reflectance(N,LAI, ALA, Hotspot, tts, tto, psi, lref, ltrans, fdir, soilref):
    N = lref.shape[0]
    refs = np.zeros(N, dtype = np.float64)
    for i in range(N):
         refs[i] = SAILHEllip(LAI, ALA, tts, tto, psi, Hotspot, lref[i], ltrans[i], soilref[i], fdir[i])
    return refs

def ProSAIL(N, Cab, Car, Cw, Cm, LAI, ALA, Hotspot, rsoil, psoil, tts, tto, psi, bands):
    bands_index = np.array(bands) - 400
    _, lref, ltrans = PROSPECT_5(N, Cab, Car, Cw, Cm, prospect_coeff)
    soilref = rsoil * (psoil * rsoil1 + (1.0 - psoil) * rsoil2)
    lref = lref[bands_index]
    ltrans = ltrans[bands_index]
    fdir = fdir_all_wvl[bands_index].flatten()
    soilref = soilref[bands_index]
    ref = generate_reflectance(N,LAI, ALA, Hotspot, tts, tto, psi, lref, ltrans, fdir, soilref)
    return ref

if __name__ == "__main__":
    # for i in range(10000):
    #     print(ProSAIL(2,40, 8, 0.01, 0.01, 3, 60, 0.3, 0.8, 0.2, 30, 0, 0, np.arange(401, 2501, 1)))
    # print(".")
    pass